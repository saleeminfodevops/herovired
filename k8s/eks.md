...
Create Cluster:
```bash
eksctl create cluster --name sample-cluster-2 --region ap-south-1 --nodegroup-name standard-workers --node-type t3.medium --nodes 2 --nodes-min 2 --nodes-max 3
```

Configuring the kubectl with eks cluster:
```bash
aws eks --region ap-south-1 update-kubeconfig --name buildflow-deployment
```
...
Delete EKS Cluster:
```bash
eksctl delete cluster --name sample-cluster-2 --region ap-south-1
```
